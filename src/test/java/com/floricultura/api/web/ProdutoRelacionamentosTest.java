 ...
 * grupos (eventos/fornecedores/clientes) derivados por nome, deduplicados e ordenados {@code nome ASC};
...
class ProdutoRelacionamentosTest {
    ...
    // ---- R-CA-10: os tres grupos derivados, deduplicados e ordenados por nome ------------------

    @Test
    void relacionamentos_comTresGrupos_deduplicaEOrdenaPorNome() throws Exception {
        Long eventoNatal = inserirEvento("Natal");
        ...
        inserirEntrada(produtoId, fornAlpha, "Alpha Flores"); // repetida → DISTINCT
        ...
        mockMvc.perform(get("/api/v1/produtos/" + produtoId + "/relacionamentos")
                ...
                // eventos (ordem nome ASC): "Dia das Maes" antes de "Natal".
                .andExpect(jsonPath("$.data.eventos.length()").value(2))
                .andExpect(jsonPath("$.data.eventos[0].id").value(eventoMaes))
                .andExpect(jsonPath("$.data.eventos[0].nome").value("Dia das Maes"))
                .andExpect(jsonPath("$.data.eventos[1].nome").value("Natal"))
                // fornecedores (ENTRADAS, DISTINCT, nome ASC): Alpha antes de Beta.
                .andExpect(jsonPath("$.data.fornecedores.length()").value(2))
                .andExpect(jsonPath("$.data.fornecedores[0].id").value(fornAlpha))
                .andExpect(jsonPath("$.data.fornecedores[0].nome").value("Alpha Flores"))
                .andExpect(jsonPath("$.data.fornecedores[1].nome").value("Beta Verde"))
                // clientes (SAIDAS, DISTINCT, nome ASC): Ana antes de Bruno.
                .andExpect(jsonPath("$.data.clientes.length()").value(2))
                .andExpect(jsonPath("$.data.clientes[0].id").value(cliAna))
                .andExpect(jsonPath("$.data.clientes[0].nome").value("Ana Silva"))
                .andExpect(jsonPath("$.data.clientes[1].nome").value("Bruno Costa"));
    }
